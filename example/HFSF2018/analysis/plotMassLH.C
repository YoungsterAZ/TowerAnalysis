...
void plotMassLH(const std::string& fname,const std::string& htag,bool notAll=false)
{
...
#include "MassProbThresholds.icc"
  ...
  int    nbins(nb);
  double xmin(v_mbins.front());
  double xmax(v_mbins.back());

  ReadMLFile indata(fname);
  printf("plotMassLH - read %zu records\n\n",indata.size());
  ...
  TH1D* h_mall = htag != "" 
    ? new TH1D(HistHelper::Names::histName(htag,"h_mall",true).c_str(),"M^{jet} (all jets)",nbins,a_mbins)
    : new TH1D(HistHelper::Names::histName(fname,"h_mall").c_str(),"M^{jet} (all jets)",    nbins,a_mbins);
  TH1D* h_msig = htag != "" 
    ? new TH1D(HistHelper::Names::histName(htag,"h_msig",true).c_str(),"M^{jet} (signal jets)",nbins,a_mbins)
    : new TH1D(HistHelper::Names::histName(fname,"h_msig").c_str(),"M^{jet} (signal jets)",    nbins,a_mbins);
  TH1D* h_mbck = htag != "" 
    ? new TH1D(HistHelper::Names::histName(htag,"h_mbck",true).c_str(),"M^{jet} (pile-up jets)",nbins,a_mbins)
    : new TH1D(HistHelper::Names::histName(fname,"h_mbck").c_str(),"M^{jet} (pile-up jets)",    nbins,a_mbins);
  ...
  std::vector<TH2D*> d_mave_all; std::vector<TH2D*> d_mave_sig; std::vector<TH2D*> d_mave_bck; 
  for ( size_t i(1); i<lThresholds.size(); ++i ) { 
    ...
    //
    int nydb(2000); double ymin(0.); double ymax(200.);
    d_mave_all.push_back(new TH2D(hname.c_str(),"average variable",nbins,a_mbins,nydb,ymin,ymax)); d_mave_all.back()->Sumw2();
    d_mave_sig.push_back(new TH2D(sname.c_str(),"average variable",nbins,a_mbins,nydb,ymin,ymax)); d_mave_sig.back()->Sumw2();
    d_mave_bck.push_back(new TH2D(bname.c_str(),"average variable",nbins,a_mbins,nydb,ymin,ymax)); d_mave_bck.back()->Sumw2();
  }
  ...
  for ( ; fiter != indata.end(); ++fiter ) { 
    double m(indata.m(fiter)); double seff(indata.psig(fiter));
    // MC truth references
    h_mall->Fill(m);
    if ( indata.isSignal(fiter) ) { h_msig->Fill(m); } else { h_mbck->Fill(m); }
    // in signal efficiency bins
    int ib = h_probBins->FindFixBin(seff);
    if ( ib < 1 && ib > h_probBins->GetNbinsX() ) { continue; }
    size_t ih(0);
    //    printf("plotMassLH - jet width %5.3f signal probability %5.3f isSignal %i probability bin %2i contributes to bins { ",w,seff,indata.tag(fiter),ib);
    for ( size_t ih(0); ih<(size_t)ib; ++ih ) { 
      //      printf("%i ",(int)ih+1);
      allHists.at(ih)->Fill(m); d_mave_all.at(ih)->Fill(m,m);
      if ( indata.isSignal(fiter) ) { sigHists.at(ih)->Fill(m); d_mave_sig.at(ih)->Fill(m,m); } else { bckHists.at(ih)->Fill(m); d_mave_bck.at(ih)->Fill(m,m); }
    }
    //    printf("}\n");
  }
  ...
  for ( auto hp : plotAllList ) { plotfall.push_back(HistHelper::Hist::histRatio(hp,h_mall)); }
  for ( auto hp : plotSigList ) { plotfsig.push_back(HistHelper::Hist::histRatio(hp,h_mall)); }
  for ( auto hp : plotBckList ) { plotfbck.push_back(HistHelper::Hist::histRatio(hp,h_mall)); }
  printf("   (*) calculated ratios\n");

  // find binning for frame
  //  int ilb = h_mall->GetXaxis()->FindFixBin(400.); 
  //  double xfmax(h_mall->GetXaxis()->GetBinLowEdge(ilb));
  ...
  _frame->GetXaxis()->SetTitle("m_{jet} [GeV]");
  if ( !notAll ) { 
    _frame->GetYaxis()->SetTitle("N_{P_{signal}^{jet} #geq P_{min}}^{#kappa}(m_{jet})/N_{P_{signal}^{jet} #geq 0}^{all jets}(m_{jet})");
  } else {
    _frame->GetYaxis()->SetTitle("N_{P_{signal}^{jet} #geq P_{min}}^{#kappa}(m_{jet})/N_{P_{signal}^{jet} #geq 0}^{all jets}(m_{jet})");
  }
  ...
  printf("plotMassLH - scale factor %.3f y-ccordinate %.3f window height %.3f\n",yspan,yref,1.-(gStyle->GetPadBottomMargin()+gStyle->GetPadTopMargin()));
  // inset plot
  ...
  h_mbck->SetLineWidth(1); h_mbck->SetLineStyle(lsbck); h_mbck->DrawCopy("same hist");
  h_msig->SetLineWidth(1); h_msig->SetLineStyle(lssig); h_msig->DrawCopy("same hist");
  h_mall->SetLineWidth(1); h_mall->SetLineStyle(lsall); h_mall->DrawCopy("same hist");
  ...
  printf("plotMassLH - legend (x0,y0,x1,y1) = (%5.3f,%5.3f,%5.3f,%5.3f) w = %5.3f h = %5.3f\n",xleg,xleg+xwdt,yleg,yleg+ywdt,xwdt,ywdt);
  ...
  for ( auto fh(plotfall.begin()); fh!=plotfall.end(); ++fh, ++ip ) { 
    ...
    TGraphAsymmErrors* pg = HistHelper::Hist::graph(d_mave_all[allIdx.at(ip)],"_aname",2);
    int npts = pg->GetN();                     // number of points ingraph (# bins on pt axis)
    std::vector<double> mave((size_t)npts,0.); // pt averages
    std::vector<double> mel((size_t)npts,0.);  // down error
    std::vector<double> meh((size_t)npts,0.);  // up error
    for ( size_t i(0); i<mave.size(); ++i ) { 
      int j(i+1); double xl(d_mave_all[allIdx.at(ip)]->GetXaxis()->GetBinLowEdge(j)); double xh(d_mave_all[allIdx.at(ip)]->GetXaxis()->GetBinUpEdge(j));
      mave[i] = pg->GetY()[i]; 
      if ( mave[i] == 0. ) { mave[i] = (xl+xh)/2.; }
      mel[i] = mave.at(i)-xl;
      meh[i] = xh-mave.at(i);
    }
    // 
    ...
    // filter
    if ( HistHelper::Data::compress(rave,rael,raeh,mave,mel,meh) ) {
      // create graph
      TGraphAsymmErrors* pgs = new TGraphAsymmErrors((int)mave.size(),&mave[0],&rave[0],&mel[0],&meh[0],&rael[0],&raeh[0]);
  ...
  for ( auto fh(plotfsig.begin()); fh!=plotfsig.end(); ++fh, ++ip ) { 
    // pt values (x-axis values)
    TGraphAsymmErrors* pg = HistHelper::Hist::graph(d_mave_sig[sigIdx.at(ip)],"_aname",2);
    int npts = pg->GetN();                      // number of points ingraph (# bins on pt axis)
    std::vector<double> mave((size_t)npts,0.); // pt averages
    std::vector<double> mel((size_t)npts,0.);  // down error
    std::vector<double> meh((size_t)npts,0.);  // up error
    for ( size_t i(0); i<mave.size(); ++i ) { 
      int j(i+1); double xl(d_mave_sig[sigIdx.at(ip)]->GetXaxis()->GetBinLowEdge(j)); double xh(d_mave_sig[sigIdx.at(ip)]->GetXaxis()->GetBinUpEdge(j));
      mave[i] = pg->GetY()[i]; 
      if ( mave[i] == 0. ) { mave[i] = (xl+xh)/2.; }
      mel[i] = mave.at(i)-xl; //pg->GetEYlow()[i]; 
      meh[i] = xh-mave.at(i); //pg->GetEYhigh()[i]; 
    }
    // 
    std::vector<double> rave(npts,0.); std::vector<double> rael(npts,0.); std::vector<double> raeh(npts,0.);
    size_t iptb(0);
  ...
  for ( auto fh(plotfbck.begin()); fh!=plotfbck.end(); ++fh, ++ip ) { 
    // pt values (x-axis values)
    TGraphAsymmErrors* pg = HistHelper::Hist::graph(d_mave_bck[bckIdx.at(ip)],"_aname",2);
    int npts = pg->GetN();                      // number of points ingraph (# bins on pt axis)
    std::vector<double> mave((size_t)npts,0.); // pt averages
    std::vector<double> mel((size_t)npts,0.);  // down error
    std::vector<double> meh((size_t)npts,0.);  // up error
    for ( size_t i(0); i<mave.size(); ++i ) { 
      int j(i+1); double xl(d_mave_bck[bckIdx.at(ip)]->GetXaxis()->GetBinLowEdge(j)); double xh(d_mave_bck[bckIdx.at(ip)]->GetXaxis()->GetBinUpEdge(j));
      mave[i] = pg->GetY()[i]; 
      if ( mave[i] == 0. ) { mave[i] = (xl+xh)/2.; }
      mel[i] = mave.at(i)-xl; //pg->GetEYlow()[i]; 
      meh[i] = xh-mave.at(i); //pg->GetEYhigh()[i]; 
  ...
  printf("plotMassLH - label text size is %.3f\n",leg->GetTextSize());
