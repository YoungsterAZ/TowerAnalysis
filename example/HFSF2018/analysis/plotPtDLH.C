...
void plotPtDLH(const std::string& fname,const std::string& htag,bool notAll=false)
{
...
#include "PtDProbThresholds.icc"
  ...
  printf("plotPtDLH - read %zu records\n\n",indata.size());
  ...
  TH1D* h_ptdall = htag != "" 
    ? new TH1D(HistHelper::Names::histName(htag,"h_ptdall",true).c_str(),"P_{T}^{D} (all jets)",nbins,xmin,xmax)
    : new TH1D(HistHelper::Names::histName(fname,"h_ptdall").c_str(),"P_{T}^{D} (all jets)",    nbins,xmin,xmax);
  TH1D* h_ptdsig = htag != "" 
    ? new TH1D(HistHelper::Names::histName(htag,"h_ptdsig",true).c_str(),"P_{T}^{D} (signal jets)",nbins,xmin,xmax)
    : new TH1D(HistHelper::Names::histName(fname,"h_ptdsig").c_str(),"P_{T}^{D} (signal jets)",    nbins,xmin,xmax);
  TH1D* h_ptdbck = htag != "" 
    ? new TH1D(HistHelper::Names::histName(htag,"h_ptdbck",true).c_str(),"P_{T}^{D} (pile-up jets)",nbins,xmin,xmax)
    : new TH1D(HistHelper::Names::histName(fname,"h_ptdbck").c_str(),"P_{T}^{D} (pile-up jets)",    nbins,xmin,xmax);
  ...
  std::vector<TH2D*> d_ptdave_all; std::vector<TH2D*> d_ptdave_sig; std::vector<TH2D*> d_ptdave_bck; 
  for ( size_t i(1); i<lThresholds.size(); ++i ) { 
    ...
    //
    d_ptdave_all.push_back(new TH2D(hname.c_str(),"average variable",nbins,xmin,xmax,100*nbins,xmin,xmax)); d_ptdave_all.back()->Sumw2();
    d_ptdave_sig.push_back(new TH2D(sname.c_str(),"average variable",nbins,xmin,xmax,100*nbins,xmin,xmax)); d_ptdave_sig.back()->Sumw2();
    d_ptdave_bck.push_back(new TH2D(bname.c_str(),"average variable",nbins,xmin,xmax,100*nbins,xmin,xmax)); d_ptdave_bck.back()->Sumw2();
  }
  ...
  for ( ; fiter != indata.end(); ++fiter ) { 
    double ptd(indata.ptd(fiter)); double seff(indata.psig(fiter));
    // MC truth references
    h_ptdall->Fill(ptd);
    if ( indata.isSignal(fiter) ) { h_ptdsig->Fill(ptd); } else { h_ptdbck->Fill(ptd); }
    // in signal efficiency bins
    int ib = h_probBins->FindFixBin(seff);
    if ( ib < 1 && ib > h_probBins->GetNbinsX() ) { continue; }
    size_t ih(0);
    //    printf("plotPtDLH - jet width %5.3f signal probability %5.3f isSignal %i probability bin %2i contributes to bins { ",w,seff,indata.tag(fiter),ib);
    for ( size_t ih(0); ih<(size_t)ib; ++ih ) { 
      //      printf("%i ",(int)ih+1);
      allHists.at(ih)->Fill(ptd); d_ptdave_all.at(ih)->Fill(ptd,ptd);
      if ( indata.isSignal(fiter) ) { sigHists.at(ih)->Fill(ptd); d_ptdave_sig.at(ih)->Fill(ptd,ptd); } else { bckHists.at(ih)->Fill(ptd); d_ptdave_bck.at(ih)->Fill(ptd,ptd); }
    }
    //    printf("}\n");
  }
  ...
  for ( auto hp : plotAllList ) { plotfall.push_back(HistHelper::Hist::histRatio(hp,h_ptdall)); }
  for ( auto hp : plotSigList ) { plotfsig.push_back(HistHelper::Hist::histRatio(hp,h_ptdall)); }
  for ( auto hp : plotBckList ) { plotfbck.push_back(HistHelper::Hist::histRatio(hp,h_ptdall)); }
  printf("   (*) calculated ratios\n");

  // find binning for frame
  //  int ilb = h_ptdall->GetXaxis()->FindFixBin(400.); 
  //  double xfmax(h_ptdall->GetXaxis()->GetBinLowEdge(ilb));
  TH1D* _frame = new TH1D(HistHelper::Names::histName(fname,"_frame").c_str(),"_frame",h_ptdall->GetNbinsX(),h_ptdall->GetXaxis()->GetXmin(),h_ptdall->GetXaxis()->GetXmax());
  ...
  if ( !notAll ) { 
    _frame->GetYaxis()->SetTitle("N_{P_{signal}^{jet} #geq P_{min}}^{#kappa}(P_{T}^{D})/N_{P_{signal}^{jet} #geq 0}^{all jets}(P_{T}^{D})");
  } else {
    _frame->GetYaxis()->SetTitle("N_{P_{signal}^{jet} #geq P_{min}}^{#kappa}(P_{T}^{D})/N_{P_{signal}^{jet} #geq 0}^{all jets}(P_{T}^{D})");
  }
  ...
  printf("plotPtDLH - scale factor %.3f y-ccordinate %.3f window height %.3f\n",yspan,yref,1.-(gStyle->GetPadBottomMargin()+gStyle->GetPadTopMargin()));
  // inset plot
  ...
  h_ptdbck->SetLineWidth(1); h_ptdbck->SetLineStyle(lsbck); h_ptdbck->DrawCopy("same hist");
  h_ptdsig->SetLineWidth(1); h_ptdsig->SetLineStyle(lssig); h_ptdsig->DrawCopy("same hist");
  h_ptdall->SetLineWidth(1); h_ptdall->SetLineStyle(lsall); h_ptdall->DrawCopy("same hist");
  ...
  printf("plotPtDLH - legend (x0,y0,x1,y1) = (%5.3f,%5.3f,%5.3f,%5.3f) w = %5.3f h = %5.3f\n",xleg,xleg+xwdt,yleg,yleg+ywdt,xwdt,ywdt);
  ...
  for ( auto fh(plotfall.begin()); fh!=plotfall.end(); ++fh, ++ip ) { 
    ...
    TGraphAsymmErrors* pg = HistHelper::Hist::graph(d_ptdave_all[allIdx.at(ip)],"_aname",2);
    int npts = pg->GetN();                     // number of points ingraph (# bins on pt axis)
    std::vector<double> ptdave((size_t)npts,0.); // pt averages
    std::vector<double> ptdel((size_t)npts,0.);  // down error
    std::vector<double> ptdeh((size_t)npts,0.);  // up error
    for ( size_t i(0); i<ptdave.size(); ++i ) { 
      int j(i+1); double xl(d_ptdave_all[allIdx.at(ip)]->GetXaxis()->GetBinLowEdge(j)); double xh(d_ptdave_all[allIdx.at(ip)]->GetXaxis()->GetBinUpEdge(j));
      ptdave[i] = pg->GetY()[i]; 
      if ( ptdave[i] == 0. ) { ptdave[i] = (xl+xh)/2.; }
      ptdel[i] = ptdave.at(i)-xl;
      ptdeh[i] = xh-ptdave.at(i);
    }
    // 
    ...
    printf("   (i) number of data points %2zu (x-vector) %2zu (y-vector) (pre-filter)\n",ptdave.size(),rave.size());
    // filter
    if ( HistHelper::Data::compress(rave,rael,raeh,ptdave,ptdel,ptdeh) ) {
      // create graph
      TGraphAsymmErrors* pgs = new TGraphAsymmErrors((int)ptdave.size(),&ptdave[0],&rave[0],&ptdel[0],&ptdeh[0],&rael[0],&raeh[0]);
  ...
  if ( !notAll ) { 
    ...
    TLegendEntry* l0 = ltx->AddEntry(gall,"all jets","l"); l0->SetTextFont(42);
  }
  ...
  for ( auto fh(plotfsig.begin()); fh!=plotfsig.end(); ++fh, ++ip ) { 
    // pt values (x-axis values)
    TGraphAsymmErrors* pg = HistHelper::Hist::graph(d_ptdave_sig[sigIdx.at(ip)],"_aname",2);
    int npts = pg->GetN();                      // number of points ingraph (# bins on pt axis)
    std::vector<double> ptdave((size_t)npts,0.); // pt averages
    std::vector<double> ptdel((size_t)npts,0.);  // down error
    std::vector<double> ptdeh((size_t)npts,0.);  // up error
    for ( size_t i(0); i<ptdave.size(); ++i ) { 
      int j(i+1); double xl(d_ptdave_sig[sigIdx.at(ip)]->GetXaxis()->GetBinLowEdge(j)); double xh(d_ptdave_sig[sigIdx.at(ip)]->GetXaxis()->GetBinUpEdge(j));
      ptdave[i] = pg->GetY()[i]; 
      if ( ptdave[i] == 0. ) { ptdave[i] = (xl+xh)/2.; }
      ptdel[i] = ptdave.at(i)-xl; //pg->GetEYlow()[i]; 
      ptdeh[i] = xh-ptdave.at(i); //pg->GetEYhigh()[i]; 
    }
    // 
    std::vector<double> rave(npts,0.); std::vector<double> rael(npts,0.); std::vector<double> raeh(npts,0.);
    size_t iptb(0);
  ...
  for ( auto fh(plotfbck.begin()); fh!=plotfbck.end(); ++fh, ++ip ) { 
    // pt values (x-axis values)
    TGraphAsymmErrors* pg = HistHelper::Hist::graph(d_ptdave_bck[bckIdx.at(ip)],"_aname",2);
    int npts = pg->GetN();                      // number of points ingraph (# bins on pt axis)
    std::vector<double> ptdave((size_t)npts,0.); // pt averages
    std::vector<double> ptdel((size_t)npts,0.);  // down error
    std::vector<double> ptdeh((size_t)npts,0.);  // up error
    for ( size_t i(0); i<ptdave.size(); ++i ) { 
      int j(i+1); double xl(d_ptdave_bck[bckIdx.at(ip)]->GetXaxis()->GetBinLowEdge(j)); double xh(d_ptdave_bck[bckIdx.at(ip)]->GetXaxis()->GetBinUpEdge(j));
      ptdave[i] = pg->GetY()[i]; 
      if ( ptdave[i] == 0. ) { ptdave[i] = (xl+xh)/2.; }
      ptdel[i] = ptdave.at(i)-xl; //pg->GetEYlow()[i]; 
      ptdeh[i] = xh-ptdave.at(i); //pg->GetEYhigh()[i]; 
  ...
  tj.SetTextAlign(12);
  if ( !notAll ) { tj.DrawLatex(xleg+xwdt/4.+0.01,yleg-0.04,"#kappa = all"); } else { tj.DrawLatex(xleg+xwdt/4.+0.01,yleg-0.04,"all jets"); } 
  ...
  printf("plotPtDLH - label text size is %.3f\n",leg->GetTextSize());
