...
void plotAllWLH(bool notAll=false) {
...
#include "HistTags.icc"
  ...
  for ( size_t ic(0); ic < fnames.size(); ++ic ) { //fnames.size(); ++ic ) {
    ...
    //    a->SetLogx();
    plotWLH(fnames.at(ic),htags.at(ic),notAll);
    ...
    if ( !notAll ) { 
      ...
      a->Print(HistHelper::Names::pdfFileName(fnames.at(ic),"Weff_prob_notall").c_str());
      a->Print(HistHelper::Names::pngFileName(fnames.at(ic),"Weff_prob_notall").c_str());
    }
  }
