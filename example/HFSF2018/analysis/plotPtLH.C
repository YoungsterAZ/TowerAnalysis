...
void plotPtLH(const std::string& fname,const std::string& htag,bool notAll=false)
{
...
#include "PtProbThresholds.icc"
  ...
  if ( !notAll ) { 
    _frame->GetYaxis()->SetTitle("N_{P_{signal}^{jet} #geq P_{min}}^{#kappa}(p_{T})/N_{P_{signal}^{jet} #geq 0}^{all jets}(p_{T})");
  } else {
    _frame->GetYaxis()->SetTitle("N_{P_{signal}^{jet} #geq P_{min}}^{#kappa}(p_{T})/N_{P_{signal}^{jet} #geq 0}^{all jets}(p_{T})");
  }
