...
bool plotProbRels(const std::string& vname="pt",const std::string& ystr="00_15",const std::string& fname="tuple/JAPU+INCL_topoclus_tuple.root",bool scatter=false) {
...
#include "MassOverPtProbThresholds.icc"
  ...
  if ( mdata == 0 ) { 
	...
	if ( stag->at(i) ) { xsig.push_back(pdata->at(i)); ysig.push_back(prob->at(i)); dsig->Fill(pdata->at(i),prob->at(i)); 
	} else { xbck.push_back(pdata->at(i)); ybck.push_back(prob->at(i)); dbck->Fill(pdata->at(i),prob->at(i)); }
      }
    }
  } else {
    for ( size_t i(0); i<pdata->size(); ++i ) { 
	...
	if ( stag->at(i) ) { xsig.push_back(mop); ysig.push_back(prob->at(i)); dsig->Fill(mop,prob->at(i)); 
	} else { xbck.push_back(mop); ybck.push_back(prob->at(i)); dbck->Fill(mop,prob->at(i)); }
      }
    }
  }    
  ...
  if ( !scatter ) { 
    ...
    printf("plotProbRels - z-value range [%6.4f,%6.4f]\n",zmin,zmax);
  ...
  printf("plotProbRels - y-range string <%s>\n",ystr.c_str());
  if ( HistHelper::Tags::yRange(ystr,yl,yh) ) { 
    printf("plotProbRels - y-range string <%s> extracted y in [%3.1f,%3.1f], put label at (%5.3f,%5.3f) with alignment %i\n",
	   ystr.c_str(),yl,yh,prcx0,prcy0,prcta);
