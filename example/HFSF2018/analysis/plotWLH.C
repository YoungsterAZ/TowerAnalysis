...
void plotWLH(const std::string& fname,const std::string& htag,bool notAll=false)
{
...
#include "WProbThresholds.icc"
  ...
  printf("plotWLikelihood - read %zu records\n\n",indata.size());
  ...
  TH1D* h_wall = htag != "" 
    ? new TH1D(HistHelper::Names::histName(htag,"h_wall",true).c_str(),"w^{jet} (all jets)",nbins,xmin,xmax)
    : new TH1D(HistHelper::Names::histName(fname,"h_wall").c_str(),"w^{jet} (all jets)",    nbins,xmin,xmax);
  TH1D* h_wsig = htag != "" 
    ? new TH1D(HistHelper::Names::histName(htag,"h_wsig",true).c_str(),"w^{jet} (signal jets)",nbins,xmin,xmax)
    : new TH1D(HistHelper::Names::histName(fname,"h_wsig").c_str(),"w^{jet} (signal jets)",    nbins,xmin,xmax);
  TH1D* h_wbck = htag != "" 
    ? new TH1D(HistHelper::Names::histName(htag,"h_wbck",true).c_str(),"w^{jet} (pile-up jets)",nbins,xmin,xmax)
    : new TH1D(HistHelper::Names::histName(fname,"h_wbck").c_str(),"w^{jet} (pile-up jets)",    nbins,xmin,xmax);
  ...
  std::vector<TH2D*> d_wave_all; std::vector<TH2D*> d_wave_sig; std::vector<TH2D*> d_wave_bck; 
  for ( size_t i(1); i<lThresholds.size(); ++i ) { 
    ...
    //
    d_wave_all.push_back(new TH2D(hname.c_str(),"average variable",nbins,xmin,xmax,100*nbins,xmin,xmax)); d_wave_all.back()->Sumw2();
    d_wave_sig.push_back(new TH2D(sname.c_str(),"average variable",nbins,xmin,xmax,100*nbins,xmin,xmax)); d_wave_sig.back()->Sumw2();
    d_wave_bck.push_back(new TH2D(bname.c_str(),"average variable",nbins,xmin,xmax,100*nbins,xmin,xmax)); d_wave_bck.back()->Sumw2();
  }
  ...
  for ( ; fiter != indata.end(); ++fiter ) { 
    double w(indata.width(fiter)); double seff(indata.psig(fiter));
    // MC truth references
    h_wall->Fill(w);
    if ( indata.isSignal(fiter) ) { h_wsig->Fill(w); } else { h_wbck->Fill(w); }
    // in signal efficiency bins
    int ib = h_probBins->FindFixBin(seff);
    if ( ib < 1 && ib > h_probBins->GetNbinsX() ) { continue; }
    size_t ih(0);
    //    printf("plotWLH - jet width %5.3f signal probability %5.3f isSignal %i probability bin %2i contributes to bins { ",w,seff,indata.tag(fiter),ib);
    for ( size_t ih(0); ih<(size_t)ib; ++ih ) { 
      //      printf("%i ",(int)ih+1);
      allHists.at(ih)->Fill(w); d_wave_all.at(ih)->Fill(w,w);
      if ( indata.isSignal(fiter) ) { sigHists.at(ih)->Fill(w); d_wave_sig.at(ih)->Fill(w,w); } else { bckHists.at(ih)->Fill(w); d_wave_bck.at(ih)->Fill(w,w); }
    }
    //    printf("}\n");
  }
  ...
  for ( auto hp : plotAllList ) { plotfall.push_back(HistHelper::Hist::histRatio(hp,h_wall)); }
  for ( auto hp : plotSigList ) { plotfsig.push_back(HistHelper::Hist::histRatio(hp,h_wall)); }
  for ( auto hp : plotBckList ) { plotfbck.push_back(HistHelper::Hist::histRatio(hp,h_wall)); }
  printf("   (*) calculated ratios\n");

  // find binning for frame
  //  int ilb = h_wall->GetXaxis()->FindFixBin(400.); 
  //  double xfmax(h_wall->GetXaxis()->GetBinLowEdge(ilb));
  TH1D* _frame = new TH1D(HistHelper::Names::histName(fname,"_frame").c_str(),"_frame",h_wall->GetNbinsX(),h_wall->GetXaxis()->GetXmin(),h_wall->GetXaxis()->GetXmax());
  ...
  _frame->GetXaxis()->SetTitle("w_{jet}");
  if ( !notAll ) { 
    _frame->GetYaxis()->SetTitle("N_{P_{signal}^{jet} #geq P_{min}}(w_{jet})^{#kappa}/N_{P_{signal}^{jet} #geq 0}^{all jets}(w_{jet})");
  } else { 
    _frame->GetYaxis()->SetTitle("N_{P_{signal}^{jet} #geq P_{min}}(w_{jet})^{#kappa}/N_{P_{signal}^{jet} #geq 0}^{all jets}(w_{jet})");
  }
  ...
  printf("plotWLH - scale factor %.3f y-ccordinate %.3f window height %.3f\n",yspan,yref,1.-(gStyle->GetPadBottomMargin()+gStyle->GetPadTopMargin()));
  // inset plot
  ...
  h_wbck->SetLineWidth(1); h_wbck->SetLineStyle(lsbck); h_wbck->DrawCopy("same hist");
  h_wsig->SetLineWidth(1); h_wsig->SetLineStyle(lssig); h_wsig->DrawCopy("same hist");
  h_wall->SetLineWidth(1); h_wall->SetLineStyle(lsall); h_wall->DrawCopy("same hist");
  ...
  printf("plotWLH - legend (x0,y0,x1,y1) = (%5.3f,%5.3f,%5.3f,%5.3f) w = %5.3f h = %5.3f\n",xleg,xleg+xwdt,yleg,yleg+ywdt,xwdt,ywdt);
  ...
  for ( auto fh(plotfall.begin()); fh!=plotfall.end(); ++fh, ++ip ) { 
    ...
    TGraphAsymmErrors* pg = HistHelper::Hist::graph(d_wave_all[allIdx.at(ip)],"_aname",2);
    int npts = pg->GetN();                     // number of points ingraph (# bins on pt axis)
    std::vector<double> wave((size_t)npts,0.); // pt averages
    std::vector<double> wel((size_t)npts,0.);  // down error
    std::vector<double> weh((size_t)npts,0.);  // up error
    for ( size_t i(0); i<wave.size(); ++i ) { 
      int j(i+1); double xl(d_wave_all[allIdx.at(ip)]->GetXaxis()->GetBinLowEdge(j)); double xh(d_wave_all[allIdx.at(ip)]->GetXaxis()->GetBinUpEdge(j));
      wave[i] = pg->GetY()[i]; 
      if ( wave[i] == 0. ) { wave[i] = (xl+xh)/2.; }
      wel[i] = wave.at(i)-xl;
      weh[i] = xh-wave.at(i);
    }
    // 
    ...
    // filter
    if ( HistHelper::Data::compress(rave,rael,raeh,wave,wel,weh) ) {
      // create graph
      TGraphAsymmErrors* pgs = new TGraphAsymmErrors((int)wave.size(),&wave[0],&rave[0],&wel[0],&weh[0],&rael[0],&raeh[0]);
  ...
  for ( auto fh(plotfsig.begin()); fh!=plotfsig.end(); ++fh, ++ip ) { 
    // pt values (x-axis values)
    TGraphAsymmErrors* pg = HistHelper::Hist::graph(d_wave_sig[sigIdx.at(ip)],"_aname",2);
    int npts = pg->GetN();                      // number of points ingraph (# bins on pt axis)
    std::vector<double> wave((size_t)npts,0.); // pt averages
    std::vector<double> wel((size_t)npts,0.);  // down error
    std::vector<double> weh((size_t)npts,0.);  // up error
    for ( size_t i(0); i<wave.size(); ++i ) { 
      int j(i+1); double xl(d_wave_sig[sigIdx.at(ip)]->GetXaxis()->GetBinLowEdge(j)); double xh(d_wave_sig[sigIdx.at(ip)]->GetXaxis()->GetBinUpEdge(j));
      wave[i] = pg->GetY()[i]; 
      if ( wave[i] == 0. ) { wave[i] = (xl+xh)/2.; }
      wel[i] = wave.at(i)-xl; //pg->GetEYlow()[i]; 
      weh[i] = xh-wave.at(i); //pg->GetEYhigh()[i]; 
    }
    // 
    std::vector<double> rave(npts,0.); std::vector<double> rael(npts,0.); std::vector<double> raeh(npts,0.);
    size_t iptb(0);
  ...
  for ( auto fh(plotfbck.begin()); fh!=plotfbck.end(); ++fh, ++ip ) { 
    // pt values (x-axis values)
    TGraphAsymmErrors* pg = HistHelper::Hist::graph(d_wave_bck[bckIdx.at(ip)],"_aname",2);
    int npts = pg->GetN();                      // number of points ingraph (# bins on pt axis)
    std::vector<double> wave((size_t)npts,0.); // pt averages
    std::vector<double> wel((size_t)npts,0.);  // down error
    std::vector<double> weh((size_t)npts,0.);  // up error
    for ( size_t i(0); i<wave.size(); ++i ) { 
      int j(i+1); double xl(d_wave_bck[bckIdx.at(ip)]->GetXaxis()->GetBinLowEdge(j)); double xh(d_wave_bck[bckIdx.at(ip)]->GetXaxis()->GetBinUpEdge(j));
      wave[i] = pg->GetY()[i]; 
      if ( wave[i] == 0. ) { wave[i] = (xl+xh)/2.; }
      wel[i] = wave.at(i)-xl; //pg->GetEYlow()[i]; 
      weh[i] = xh-wave.at(i); //pg->GetEYhigh()[i]; 
  ...
  printf("plotWLH - label text size is %.3f\n",leg->GetTextSize());
