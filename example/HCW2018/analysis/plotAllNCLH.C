...
void plotAllNCLH(bool notAll=false) {
...
#include "HistTags.icc"
  ...
  for ( size_t ic(0); ic < fnames.size(); ++ic ) { //fnames.size(); ++ic ) {
    ...
    //a->SetLogx();
    plotNCLH(fnames.at(ic),htags.at(ic),notAll);
    ...
    if ( !notAll ) { 
      ...
      a->Print(HistHelper::Names::pdfFileName(fnames.at(ic),"NCeff_prob_notall").c_str());
      a->Print(HistHelper::Names::pngFileName(fnames.at(ic),"NCeff_prob_notall").c_str());
    }
  }
