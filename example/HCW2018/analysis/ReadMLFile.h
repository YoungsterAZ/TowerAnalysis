...
private:
  ...
  bool f_readFile() {
    ...
    if ( _printflg ) { printf("ReadMLFile::f_readFile(...) - read %zu entries from file \042%s\042\n",_store.size(),_fname.c_str()); }
    f.close();
    return !_store.empty();
  }
...
public: 
  ...
  ReadMLFile(const std::string& fname,bool printflg=false) : _fname(fname), _printflg(printflg) { f_readFile(); }
  ...
  bool   isSignal(store_t::const_iterator fiter) const { return fiter != _store.end() && fiter->tag == 1;     }
  ...
  bool   isSignal(const tuple_t& tuple) const { return tuple.tag == 1; }

  template<class H>
  void fillPt(H* h_sig,H* h_bck) {
    for ( auto fiter(begin()); fiter!=end(); ++fiter ) { 
      if ( isSignal(fiter) ) { h_sig->Fill(pt(fiter)); } else { h_bck->Fill(pt(fiter)); }
    }
  }
};
#endif
