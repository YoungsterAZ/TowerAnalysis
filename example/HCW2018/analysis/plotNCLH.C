...
void plotNCLH(const std::string& fname,const std::string& htag,bool notAll=false)
{
...
#include "NCProbThresholds.icc"
  ...
  for ( auto hp : plotSigList ) { plotfsig.push_back(HistHelper::Hist::histRatio(hp,h_ncall)); }
  for ( auto hp : plotBckList ) { plotfbck.push_back(HistHelper::Hist::histRatio(hp,h_ncall)); }
  ...
  if ( !notAll ) { 
    _frame->GetYaxis()->SetTitle("N_{P_{signal}^{jet} #geq P_{min}}^{#kappa}(N_{const}^{jet})/N_{P_{signal}^{jet} #geq 0}^{all jets}(N_{const}^{jet})");
  } else {
    _frame->GetYaxis()->SetTitle("N_{P_{signal}^{jet} #geq P_{min}}^{#kappa}(N_{const}^{jet})/N_{P_{signal}^{jet} #geq 0}^{all jets}(N_{const}^{jet})");
  }
