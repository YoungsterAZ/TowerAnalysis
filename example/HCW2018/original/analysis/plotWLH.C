...
void plotWLH(const std::string& fname,const std::string& htag)
{
...
#include "WProbThresholds.icc"
  ...
  for ( auto hp : plotSigList ) { plotfsig.push_back(HistHelper::Hist::histRatio(hp,h_wsig)); }
  for ( auto hp : plotBckList ) { plotfbck.push_back(HistHelper::Hist::histRatio(hp,h_wbck)); }
