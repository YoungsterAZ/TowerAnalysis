...
void plotPtDLH(const std::string& fname,const std::string& htag)
{
...
#include "WProbThresholds.icc"
  ...
  for ( ; fiter != indata.end(); ++fiter ) { 
    ...
    // MC truth references
    h_ptdall->Fill(w);
    if ( indata.isSignal(fiter) ) { h_ptdsig->Fill(w); } else { h_ptdbck->Fill(w); }
    ...
    for ( size_t ih(0); ih<(size_t)ib; ++ih ) { 
      //      printf("%i ",(int)ih+1);
      allHists.at(ih)->Fill(w); d_ptdave_all.at(ih)->Fill(w,w);
      if ( indata.isSignal(fiter) ) { sigHists.at(ih)->Fill(w); d_ptdave_sig.at(ih)->Fill(w,w); } else { bckHists.at(ih)->Fill(w); d_ptdave_bck.at(ih)->Fill(w,w); }
    }
    //    printf("}\n");
  }
  ...
  for ( auto hp : plotSigList ) { plotfsig.push_back(HistHelper::Hist::histRatio(hp,h_ptdsig)); }
  for ( auto hp : plotBckList ) { plotfbck.push_back(HistHelper::Hist::histRatio(hp,h_ptdbck)); }
  ...
  for ( auto fh(plotfall.begin()); fh!=plotfall.end(); ++fh, ++ip ) { 
    ...
    for ( auto pti : ptdave ) { 
      ...
      rave[iptb] = y; 
      if ( y+dy > 1. ) { raeh[iptb] = 0.; } else { raeh[itpb] = dy; } 
      if ( y-dy < 0. ) { rael[iptb] = y;  } else { rael[itpb] = dy; } 
      ++iptb;
    }
