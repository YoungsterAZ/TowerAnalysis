...
void plotMassLH(const std::string& fname,const std::string& htag)
{
...
#include "MassProbThresholds.icc"
  ...
  for ( auto hp : plotSigList ) { plotfsig.push_back(HistHelper::Hist::histRatio(hp,h_msig)); }
  for ( auto hp : plotBckList ) { plotfbck.push_back(HistHelper::Hist::histRatio(hp,h_mbck)); }
