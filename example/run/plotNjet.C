...
void plotNjet()
{
  ...
  // open the file
  TFile* f = new TFile("vbf2600_topocl_rapgap_2.0.root","READ");
  // check if successful
...
->Print("tsig_cjets_ini.pdf");
  ...
  TCanvas* ct = new TCanvas("tsigJetsFin","Signal in Jet Timing After Cut 1, TopoCluster Jets");
  ct->SetLogy();
...
->Print("tsig_cjets_matched.pdf");
  ...
  /////////////////////////////////////////////
  //39. Timing vs. rapidity, Matched CaloJets//
  /////////////////////////////////////////////
  ...
  cam->Print("tjet_rap_cmatched.pdf");

  ////////////////////////////
  //40. Jet Timing, CaloJets//
  ////////////////////////////


  // find histos 
  TH1D* h_t_cjet_ini2   = (TH1D*)f->FindObjectAny("Calo/Initial/h_tjet");
  TH1D* h_t_cjet_rho2   = (TH1D*)f->FindObjectAny("Calo/PUArea/h_tjet");
  TH1D* h_t_cjet_match2 = (TH1D*)f->FindObjectAny("MatchedCalo/Final/h_tjet");
  if ( h_t_cjet_ini2 ==  0 ) { printf("cannot find jet t histo!\n"); return; }

  TCanvas* can = new TCanvas("tJetsAll","Jet Timing, All TopoCluster Jets");
  can->SetLogy();

  TH1D* _frame19 = new TH1D("_frame19","_frame19",100,0.,120.);

  _frame19->SetTitle(0);
  _frame19->GetXaxis()->SetTitle("t^{jet} [ns]");
  _frame19->GetXaxis()->SetLimits(-110,80);
  _frame19->GetYaxis()->SetTitle("#partialN/#partialt ns^{-1}");
  _frame19->SetMinimum(0.5);
  _frame19->SetMaximum(1000000.);
  _frame19->DrawCopy("axis");

  h_t_cjet_ini2->SetLineColor(kRed);
  h_t_cjet_ini2->SetFillColorAlpha(kViolet, 0.35);                 
  h_t_cjet_ini2->SetMarkerStyle(0); 

  h_t_cjet_rho2->SetLineColor(kBlue);
  h_t_cjet_rho2->SetFillColorAlpha(kSpring, 0.35);                 
  h_t_cjet_rho2->SetMarkerStyle(0);  

  h_t_cjet_match2->SetLineColor(kMagenta);
  h_t_cjet_match2->SetFillColorAlpha(kAzure, 0.35);                 
  h_t_cjet_match2->SetMarkerStyle(0);                                     

  TLegend* lg23 = new TLegend(0.5,0.75,0.8,0.9);
  lg23->SetTextSize(_frame->GetXaxis()->GetLabelSize());
  lg23->AddEntry(h_t_cjet_ini2,"t_{cjet}^{Initial}","lf");
  lg23->AddEntry(h_t_cjet_rho2,"t_{cjet}^{rho}","lf");
  lg23->AddEntry(h_t_cjet_match2,"t_{cjet}^{Matched}","lf");
  lg23->Draw("same");

  TH1D* h_t_cjet_acopy = new TH1D(*h_t_cjet_match2);
  h_t_cjet_acopy->SetFillColor(kAzure);
  h_t_cjet_acopy->DrawCopy("hist same");



  _frame19->DrawCopy("axis same");

  can->Print("tcjets_all.pdf");
