...
void JetAnalysisExample::Loop(Long64_t nreq)
{
  ...
  TFile* of = (TFile*)0;
  bool doWrite(false);
  if ( !Analysis::Config::Output::file && Analysis::Config::Output::file != "" ) { 
    AH::openFile(Analysis::Config::Output::file);
    if ( of == 0 ) {
      PRINT_ERROR(_mname, "could not open requested output file \042%s\042 - fatal", Analysis::Config::Output::file.c_str());
      break;
    } 
    doWrite = true; 
  }
  ...
  if ( doWrite ) { 
    PRINT_INFO( _mname+"::Output", "wrote %i histograms to file \042%s\042", AH::writeHists(), AH::getFileName() );
    AH::closeFile();
  }
}
